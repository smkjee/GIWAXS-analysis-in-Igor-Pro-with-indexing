...
function CCDGraph([minqy,maxqy,minqz,maxqz,colors,minval, maxval,name])
	...
	maxqz = paramisdefault(maxqz) ? 2.1 : maxqz
	...
	if(dimoffset(CCDImageToConvert,0)<-1.2)
		minqy = paramisdefault(minqy) ? 0.15 : minqy
		maxqy = paramisdefault(maxqy) ? -2.1 : maxqy
	else
		minqy = paramisdefault(minqy) ? -0.15 : minqy
		maxqy = paramisdefault(maxqy) ? 2.1 : maxqy
	endif
	string dataname = (name+"_2D")
	duplicate/o CCDImageToConvert $dataname
	wave data = $dataname
	...
	for(j=0;j<101;j+=1)
		if(GIWAXSTIX[j]*2 == round(GIWAXSTIX[j]*2))
			GIWAXSTIXnames[j][0] = Num2str(abs(GIWAXSTIX*1))
...
//	Added for Bin Li figure
//	ModifyGraph margin(left)=57,margin(bottom)=48,width=141.732,height=141.732
//	ModifyGraph fStyle(left)=1
//	ModifyGraph fStyle=1
//	Added for Bin Li figure
	Label left "Q\\Bz\\M (Å\\S-1\\M)"
	Label bottom "Q\\Bxy\\M (Å\\S-1\\M)"
