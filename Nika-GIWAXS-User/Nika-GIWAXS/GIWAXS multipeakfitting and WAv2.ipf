...
function simplepeakfittopgraph()
	...
	variable error = MPF2_AutoMPFit(name, "Gauss", "peakCoef%d", "LogPoly5", "Background", wavenames, xwaves, 5 ,minAutoFindFraction=.01,noiseEst=7,smFact=12)
	...
	for(i=0;i<num;i+=1)
		...
		for(j=0;j<gnumpeaks;j+=1)
			...
			goodfit *= peaktest < .02 || peaktest>20 ? 0 : 1
...
//		endfor
	...
	display /k=1 /n=depthprofilingplot coherence vs timestamp
	appendtograph /w=depthprofilingplot /r=size peakheight vs timestamp
	appendtograph /w=depthprofilingplot /r=loc peakloc vs timestamp
	...
	display /k=1/n=$(NAME+"_depth") peakloc vs timestamp
...
Function CloneWindow([win,newwindowname,thisfolder])
	...
	thisfolder = paramisdefault(thisfolder)? 0 : 1
	String curr_folder=GetDataFolder(1)
	if(!thisfolder)
		NewDataFolder /O/S root:$newwindowname
	endif
