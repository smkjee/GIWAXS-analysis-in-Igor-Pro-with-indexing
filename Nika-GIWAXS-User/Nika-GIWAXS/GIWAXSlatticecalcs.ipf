		...
		for(k=maxb;k>=minb;k-=1)
			for(L=maxc;L>=minc;L-=1)
				...
				if(mag<maxq && mag>0)
					...
					qz[goodcount,goodcount+1] = -mag*cosxi              //Subh changed mag to -mag
...
function setuplatticeclicking()
	...
	//ccdplotsquare()
	ccdgraph(name="GIWAXS_Lattice",minval=100,maxval=10000,minqz = 0.05,maxqz = -2.5, minqy = -2, maxqy=2.1,colors="Terrain")     //Subh change
	...
	//ModifyImage ''#0 ctab= {100,2000,Spectrum,0}
