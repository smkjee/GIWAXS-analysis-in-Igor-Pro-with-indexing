...
Function /s loadfitsfilenika(filename,path,refnum,NewWaveName)
	...
	if(fuckedI0s)
		...
		if(failed==0)
			...
			Izerolast = str2num(stringbykey("AI3Izero",header))
...
//				byteoffset = mod(v_filepos,80)
	...
	variable Izero = str2num(stringbykey("AI3Izero",header))-Izerolast
...
function flattenimage(image,yloc,wid)
	...
	variable yloc,wid
	make/o/n=2 ytrace={yloc,yloc},xtrace={0,dimsize(image,1)}
	ImageLineProfile srcWave=image, xWave=xTrace, yWave=yTrace, width=(wid)
...
//	os = os<3.4?0:1
	...
	WAVE/Z ai3=ai_3_izero, bc=Beam_current, i00=izero, timeStamp=Time_of_Day
	i0 -= Izerooff
...
//		endif
		...
		elseif( stringmatch(pdname,"AI_6_Beamstop") )
...
//				string unLogQname=
		...
		//add to plot if needed
			if(plotq)
				...
				duplicate/o rwave, $("root:"+foldername+":'ro_" + loname+"'")
				wave roff= $("root:"+foldername+":'ro_" + loname+"'")
				//duplicate/o $qname, $("root:"+foldername+":'qo_" + loname+"'")
				duplicate/o qwave, $("root:"+foldername+":'qo_" + loname+"'")
				wave qoff= $("root:"+foldername+":'qo_" + loname +"'")
				roff *=offpwr^j
				//if(logq)
				//	qoff = exp(qoff)
				//endif
				if(addtograph && !logq) //plot unlog here
					appendtograph /w=$("plot_"+gn) $("root:"+foldername+":'ro_" + loname+"'") vs $("root:"+foldername+":'qo_" + loname+"'")
...
function Gi_Options() : Panel
	...
	//phiangle=0
	dowindow /k GiOptions
	NewPanel /N=GiOptions/k=1 /W=(1117,82,1314,325) as "Grazing Incidence Options"    //Subh changed 257 to 325
	ModifyPanel fixedSize=1
