...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function NI1A_AverageDataPerUserReq(orientation,wavelengths)
	STRING ORIENTATION
	string wavelengths
...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function NI1A_CorrectDataPerUserReq(orientation,wavelengths)
	string orientation
	string wavelengths
...
Function NI1A_CheckGeometryWaves(orientation,wavelengths)		//checks if current geometry waves are OK for the input geometry
	string orientation
	string wavelengths
...
//	wave/Z Radius2DWave=root:Packages:Convert2Dto1D:Radius2DWave
	...
	if (!WaveExists(Qvector) || !WaveExists(HistogramWv) || !WaveExists(LUT))
		...
		NI1A_CreateLUT(orientation,wavelengths)					//creates 1D LUT, should not be run always....
		//NI1A_CreateQvector(orientation)				//creates 2-D Q wave does not need to be run always...
		NI1A_CreateHistogram(orientation,wavelengths)				//creates 2-D Q wave does not need to be run always...
...
//		endif		//the ones below must be run always...
	//	NI1A_CreateQvector(orientation)				//creates 2-D Q wave does not need to be run always...
		...
		if(!WaveExists(Qdistribution1D))
			NI1A_CreateLUT(orientation,wavelengths)					//creates 1D LUT, should not be run always.... Will create Qdistribution 1D vector...
		endif
		NI1A_CreateHistogram(orientation,wavelengths)				//creates 2-D Q wave does not need to be run always...
		yesno=1
	endif
...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function NI1A_CreateHistogram(orientation,wavelengths)
	string orientation
	string wavelengths
...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function NI1A_CreateQvector(orientation,wavelengths)
	string orientation
	string wavelengths
...
Function NI1A_CreateLUT(orientation,wavelengths)
	string orientation
	string wavelengths
...
Function NI1A_Create2DQWave(DataWave)
	...
	MatrixOp/O/NTHR=1 qxzwave=DataWave
	MatrixOp/O/NTHR=1 qxywave=DataWave
	MatrixOp/O/NTHR=1 Xiwave=DataWave
	MatrixOp/O/NTHR=1 thetaxzwave=DataWave
	MatrixOp/O/NTHR=1 thetaxywave=DataWave
	MatrixOp/O/NTHR=1 thetaxzpurewave=DataWave
	MatrixOp/O/NTHR=1 thetaxypurewave=DataWave
	...
	if(reflbeam==1)//direct beam
		Multithread thetaxzwave = atan( -(cos(phi)*(q - BeamCenterY)*PixelSizeY +sin(phi)*(p - BeamCenterX)*PixelSizeX )/sqrt(SampleToCCDDistance^2 +( (cos(phi)*(p - BeamCenterX)*PixelSizeX -sin(phi)*(q - BeamCenterY)*PixelSizeY))^2))/2
		Multithread thetaxzpurewave = atan( -(q - BeamCenterY)*PixelSizeY/(SampleToCCDDistance))/2
	elseif(reflbeam==2)//reflected Beam
		Multithread thetaxzwave = atan( -(cos(phi)*(q - BeamCenterY)*PixelSizeY +sin(phi)*(p - BeamCenterX)*PixelSizeX )/sqrt(SampleToCCDDistance^2 +( (cos(phi)*(p - BeamCenterX)*PixelSizeX -sin(phi)*(q - BeamCenterY)*PixelSizeY))^2))/2 - grazingangle * pi / 180 // rotate by phi angle around beamcenter
		Multithread thetaxzpurewave = atan( -(q - BeamCenterY)*PixelSizeY/SampleToCCDDistance)/2 - grazingangle * pi / 180 // rotate by phi angle around beamcenter
	else // average of the two
		Multithread thetaxzwave = atan( -(cos(phi)*(q - BeamCenterY)*PixelSizeY +sin(phi)*(p - BeamCenterX)*PixelSizeX )/sqrt(SampleToCCDDistance^2 +( (cos(phi)*(p - BeamCenterX)*PixelSizeX -sin(phi)*(q - BeamCenterY)*PixelSizeY))^2))/2 - .5 * grazingangle * pi / 180 // rotate by phi angle around beamcenter
		Multithread thetaxzpurewave = atan( -(q - BeamCenterY)*PixelSizeY/SampleToCCDDistance)/2 - .5 * grazingangle * pi / 180 // rotate by phi angle around beamcenter
	endif
	Multithread thetaxywave = atan( (cos(phi)*(p - BeamCenterX)*PixelSizeX -sin(phi)*(q - BeamCenterY)*PixelSizeY)/sqrt(SampleToCCDDistance^2 +((cos(phi)*(q - BeamCenterY)*PixelSizeY +sin(phi)*(p - BeamCenterX)*PixelSizeX ))^2))/2  // rotate by phi angle around beamcenter
	Multithread thetaxypurewave = atan( (p - BeamCenterX)*PixelSizeX/SampleToCCDDistance)/2  // rotate by phi angle around beamcenter
	...
	//Multithread qxzwave = ((4*pi)/Wavelength)*sin(atan( -(q-BeamCenterY)*PixelSizeY/(SampleToCCDDistance))/2)
	Multithread qxzwave = ((4*pi)/Wavelength)*sin(thetaxzwave)
	Multithread qywave = ((4*pi)/Wavelength)*sin(thetaxywave)
	Multithread qzwave =  qxzwave * cos(thetaxzwave)
	Multithread qxwave = qxzwave * sin(thetaxzwave)
	Multithread qxywave = sqrt(qxwave^2 + qywave^2) * sign(qywave)
	multithread xiwave = ( 180/pi ) * atan(abs(qxywave) / abs(qzwave))
	
	multithread qxypure = ((4*pi)/Wavelength)*sin(thetaxypurewave)// this is the point there the beam center is being moved - histogram assumes  
	multithread qzpure=  ((4*pi)/Wavelength)*sin(thetaxzpurewave)
...
Function NI1A_UpdateEmptyDarkListBox()		
	...
	setDataFolder root:Packages:Convert2Dto1D
		...
		ListOfAvailableDataSets=NI1A_CleanListOfFilesForTypes(ListOfAvailableDataSets,BlankFileExtension,EmptyDarkNameMatchStr)
...
Function NI1A_ImportThisOneFile(SelectedFileToLoad)
	...
	NI1A_UniversalLoader("Convert2Dto1DDataPath",SelectedFileToLoad,DataFileExtension,"CCDImageToConvert")
		...
		if (ListOf2DSampleDataNumbers[i])
			...
			if(cmpstr(extension,"BSL/SAXS")==0&&sumframes==0&&average==0)
				...
				for(u=currentframe;u<saxsframe+1;u+=1)
					...
					if(v_flag==0)
						SetVariable HookNameSV,win=NI1A_Convert2Dto1DPanel,valueColor=(0,26112,0)
						SetVariable HookNameSV,win=NI1A_Convert2Dto1DPanel,valueBackColor=(65535,65535,65535)
					else
						SetVariable HookNameSV,win=NI1A_Convert2Dto1DPanel,valueColor=(65535,65535,65535)
						SetVariable HookNameSV,win=NI1A_Convert2Dto1DPanel,valueBackColor=(52224,0,0)
					endif
				endfor
				currentframe=1
			else
...
Function NI1A_UpdateDataListBox()		
	...
	setDataFolder root:Packages:Convert2Dto1D
		...
		elseif(cmpstr(DataFileExtension, ".fits")==0)
			realExtension=".fits"
		elseif (cmpstr(DataFileExtension,"Pilatus")==0)
			SVAR PilatusFileType=root:Packages:Convert2Dto1D:PilatusFileType
			if(!cmpstr(PilatusFileType,"edf"))
				realExtension=".edf"
			elseif(!cmpstr(PilatusFileType,"tiff")||!cmpstr(PilatusFileType,"float-tiff"))
		...
		if(V_Flag)	//path exists...
			...
			ListOfAvailableCompounds=NI1A_CleanListOfFilesForTypes(ListOfAvailableCompounds,DataFileExtension,SampleNameMatchStr)
			...
			NI1A_CreateListOfFiles(ListOf2DSampleData,ListOfAvailableCompounds,realExtension,"")
...
//first data selection part
	...
	CheckBox InvertImages,pos={260,75},size={146,14},proc=NI1A_CheckProc,title="Invert 0, 0 corner?"
	...
	Button RefreshList,pos={330,90},size={100,18},proc=NI1A_ButtonProc,title="Refresh"
	Button RefreshList,help={"Refresh lisbox"}
	Button SaveCurrentToolSetting,pos={330,110},size={100,18},proc=NI1A_ButtonProc,title="Save/Load Config"
	Button SaveCurrentToolSetting,help={"Save or recall configuration of this panel"}, font="Times New Roman",fSize=11
	Button ExportDisplayedImage,pos={330,130},size={100,18},proc=NI1A_ButtonProc,title="Export image"
	Button ExportDisplayedImage,help={"Save displayed image as tiff file for future use"}, font="Times New Roman",fSize=11
	Button SaveDisplayedImage,pos={330,150},size={100,18},proc=NI1A_ButtonProc,title="Store image"
	...
	SetVariable SampleNameMatchStr,pos={10,214},size={135,18},proc=NI1A_PanelSetVarProc,title="Match :"
	...
	PopupMenu SelectEndOfRange,pos={160,239},size={211,21},proc=NI1A_PopMenuProc,title="End"
...
//tab 1 geometry and method of calibration
	...
	SetVariable BeamCenterX,limits={0,Inf,1},value= root:Packages:Convert2Dto1D:BeamCenterX
	SetVariable BeamCenterY,pos={222,400},size={160,16},proc=NI1A_PanelSetVarProc,title="Beam center"
	SetVariable BeamCenterY,limits={0,Inf,1},value= root:Packages:Convert2Dto1D:BeamCenterY
...
//tab 4 Empty, dark and pixel sensitivity
	...
	SetVariable EmptyDarkNameMatchStr,pos={245,510},size={155,18},proc=NI1A_PanelSetVarProc,title="Match :"
...
//tab 5 output conditions
	...
	CheckBox UseDspacing,pos={180,310},size={90,14},title="d space?", mode=1, proc=NI1A_CheckProc
	...
	CheckBox UseTheta,pos={260,310},size={90,14},title="2 Theta space?", mode=1, proc=NI1A_CheckProc
...
Function TransAx_CalculateHorizQaxis(w, x)
	...
	Variable x		//in pixels
		...
		if(usegrazingincidence)
			wave data = root:Packages:Convert2Dto1D:ccdimagetoconvert
			distinq = dimdelta(data,0) * (x-beamcenterx)
		elseif(UseLineProfile && (stringMatch(LineProf_CurveType,"GI_Vertical Line") || stringMatch(LineProf_CurveType,"GI_Horizontal Line")))
			// this is exception,  need to use GI geometry for conversion, All other should eb the same...
			DistInQ=NI1GI_CalculateQxyz(PixPosition,BeamCenterY,"Y") // Eliot changed this to xy rather than Y
		else
